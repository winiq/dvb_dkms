...
static void stb0899_dvbs2_config_uwp(struct stb0899_state *state)
{
	...
	stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_UWP_CNTRL3, STB0899_OFF0_UWP_CNTRL3, uwp3);
        ...
        stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_SOF_SRCH_TO, STB0899_OFF0_SOF_SRCH_TO, config->sof_search_timeout);
...
static void stb0899_dvbs2_config_csm_auto(struct stb0899_state *state)
{
	...
	reg = 0x00020019; // STB0899_READ_S2REG(STB0899_S2DEMOD, CSM_CNTRL1);
...
static void stb0899_dvbs2_set_btr_loopbw(struct stb0899_state *state)
{
	...
	if (K != 0) {
		...
		k_indirect = (wn * wn) / K;
		k_indirect = k_indirect;	  /*kindirect = kindirect 10^-6*/               // not need
...
static void stb0899_dvbs2_set_carr_freq(struct stb0899_state *state, s32 carr_freq, u32 master_clk)
{
	...
	crl_nom_freq = (1 << config->crl_nco_bits) / master_clk;
 	crl_nom_freq *= carr_freq;  
        ...
        stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_CRL_NOM_FREQ, STB0899_OFF0_CRL_NOM_FREQ, crl_nom_freq);
}
...
static void stb0899_dvbs2_btr_init(struct stb0899_state *state)
{
	...
	reg = 0x3553; //STB0899_READ_S2REG(STB0899_S2DEMOD, BTR_CNTRL);
...
static void stb0899_dvbs2_reacquire(struct stb0899_state *state)
{
	...
	reg = 0x00054802; // STB0899_READ_S2REG(STB0899_S2DEMOD, EQ_CNTRL);
	...
	// adds, grabbed from Technotrend driver
	reg = stb0899_read_reg(state, STB0899_PDELCTRL);  // ex  48 / 4A
        STB0899_SETFIELD_VAL(ALGO_SWRST, reg, 1);         // do algorithm reset
	stb0899_write_reg(state, STB0899_PDELCTRL, reg);
	reg = stb0899_read_reg(state, STB0899_PDELCTRL);
        STB0899_SETFIELD_VAL(ALGO_SWRST, reg, 0);         
	stb0899_write_reg(state, STB0899_PDELCTRL,reg);
	reg = stb0899_read_reg(state, STB0899_PDELCTRL);
        STB0899_SETFIELD_VAL(HYST_SWRST, reg, 1);         // reset hysteresis engine
	stb0899_write_reg(state, STB0899_PDELCTRL, reg); 
	reg = stb0899_read_reg(state, STB0899_PDELCTRL);
        STB0899_SETFIELD_VAL(HYST_SWRST, reg, 0);
	stb0899_write_reg(state, STB0899_PDELCTRL, reg);

	// original Manu Abraham
...
static enum stb0899_status stb0899_dvbs2_get_dmd_status(struct stb0899_state *state, int timeout)
{
	...
	if (STB0899_GETFIELD(IF_AGC_LOCK, reg))
		dprintk(state->verbose, FE_DEBUG, 1, "------------->IF AGC LOCKED !");  // not read in windows
...
//        timeout = 1; // dirty
	...
	do {
		...
		time++;
//		msleep(1);  // takes 8ms in real world
		udelay(1000);
 ...
 */
enum stb0899_status stb0899_dvbs2_algo(struct stb0899_state *state)  // FE_STB0899_DVBS2Algo
{
	struct stb0899_internal *internal = &state->internal;
	...
	stb0899_write_s2reg(state, STB0899_S2DEMOD, STB0899_BASE_IF_AGC_CNTRL, STB0899_OFF0_IF_AGC_CNTRL, reg);
        ...
        reg = STB0899_READ_S2REG(STB0899_S2DEMOD, IF_AGC_CNTRL2);  // 0x00005007 modified at end
	...
	reg = 2; //STB0899_READ_S2REG(STB0899_S2DEMOD, DMD_CNTRL2);
...
*/
	...
	if (internal->status == DVBS2_FEC_LOCK) {
		...
		reg = STB0899_READ_S2REG(STB0899_S2DEMOD, IF_AGC_CNTRL); // 0xf320
